...
namespace MyHostel_Admin.Pages.Admin_Page.Facilities
{
    public class IndexModel : PageModel
    {
        ...
        public async Task<IActionResult> OnGetAsync(string currentFilter, string searchString, int? pageIndex)
        {
            ...
            IQueryable<Facility> genresIQ = from s in context.Facilities select s;
            if (!String.IsNullOrEmpty(searchString))
            {
                genresIQ = genresIQ.Where(s => s.UtilityName.ToLower().Contains(searchString.ToLower()));
            }
            var pageSize = Configuration.GetValue("PageSize", 2);
            Facility = await PaginatedList<Facility>.CreateAsync(genresIQ.AsNoTracking(), pageIndex ?? 1, pageSize);
            return Page();
        }
    }
}
